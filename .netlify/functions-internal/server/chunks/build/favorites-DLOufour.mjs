...
import { e as useFavoritesStore, u as useSeoMeta, I as Icon } from './server.mjs';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(` 我的收藏 (${ssrInterpolate(unref(favoriteIds).length)}) </span></h1>`);
      if (unref(loading)) {
        _push(`<div style="${ssrRenderStyle({ "text-align": "center", "padding": "50px" })}"> 加载中... </div>`);
      } else if (unref(favoriteIds).length === 0) {
        _push(`<div style="${ssrRenderStyle({ "text-align": "center", "padding": "100px 20px", "color": "#666" })}">`);
        _push(ssrRenderComponent(Icon, {
          name: "mdi:movie-open",
          size: "48"
        }, null, _parent));
        _push(`<p style="${ssrRenderStyle({ "font-size": "18px" })}">还没有收藏任何电影</p>`);
        _push(ssrRenderComponent(_component_NuxtLink, {
          to: "/",
        ...
        ssrRenderList(unref(movies), (movie) => {
          _push(`<div style="${ssrRenderStyle({ "position": "relative" })}">`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: `/movie/${movie.id}`,
          ...
          _push(`<button style="${ssrRenderStyle({ "position": "absolute", "top": "10px", "right": "10px", "width": "40px", "height": "40px", "border-radius": "50%", "border": "none", "background": "rgba(239, 68, 68, 0.9)", "color": "white", "cursor": "pointer", "font-size": "20px", "display": "flex", "align-items": "center", "justify-content": "center" })}" title="取消收藏">`);
          _push(ssrRenderComponent(Icon, {
            name: "mdi:close",
            size: "24",
            color: "white"
...
//# sourceMappingURL=favorites-DLOufour.mjs.map
